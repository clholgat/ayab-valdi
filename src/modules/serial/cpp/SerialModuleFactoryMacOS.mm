// macOS implementation of Serial module
...
// Helper function to get serial ports on macOS using IOKit
static std::vector<std::string> getSerialPortsMacOS() {
    ...
    kern_return_t result = IOServiceGetMatchingServices(masterPort, matchingDict, &serialPortIterator);
    ...
    while ((serialPort = IOIteratorNext(serialPortIterator)) != 0) {
        CFStringRef devicePath = (CFStringRef)IORegistryEntryCreateCFProperty(
            serialPort,
        ...
        if (devicePath != nullptr) {
            char path[PATH_MAX];
            if (CFStringGetCString(devicePath, path, PATH_MAX, kCFStringEncodingUTF8)) {
                ports.push_back(std::string(path));
            }
...
// POSIX (macOS) serial port functions
static bool openSerialPort(const std::string& uri) {
    ...
    // Open serial port with O_RDWR | O_NOCTTY (blocking mode)
    // O_EXCL doesn't work on macOS for device files, so we skip it
    // We use blocking mode with termios timeout (like pyserial)
    int fd = open(uri.c_str(), O_RDWR | O_NOCTTY);
    if (fd < 0) {
    ...
    Valdi::Value loadModule() final {
        ...
        auto getSerialPortsFunction = Valdi::makeShared<Valdi::ValueFunctionWithCallable>(
            [](const Valdi::ValueFunctionCallContext& callContext) -> Valdi::Value {
                std::vector<std::string> ports = getSerialPortsMacOS();
        ...
        auto browseAyabMdnsFunction = Valdi::makeShared<Valdi::ValueFunctionWithCallable>(
            [](const Valdi::ValueFunctionCallContext& callContext) -> Valdi::Value {
                (void)callContext;
                auto records = browseAyabMdnsRecordsMacOS();
                auto array = Valdi::ValueArray::make(records.size());
        ...
        // requires_usb_permission_prompt function — desktop lists attached ports
        // directly via IOKit with no OS consent step, unlike Web Serial/Android.
        ...
        // prompt_websocket_url function — desktop discovers network devices via
        // browse_ayab_mdns() instead of manual URL entry.
